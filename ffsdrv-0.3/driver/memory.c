...
#pragma alloc_text(PAGE, FFSAllocateFcb)
	...
	IN PFFS_INODE         dinode)
{
	...
	{
		ULONG Totalblocks = (Fcb->dinode->di_blocks);
	...
	IN PFFS_FCB Fcb)
{
	...
	if (Fcb->ParentFcb)
	{
		ASSERT((Fcb->ParentFcb->Identifier.Type == FCB) &&
			(Fcb->ParentFcb->Identifier.Size == sizeof(FFS_FCB)));
	...
	ULONG           FileAttr)
{
	...
	if (FFSGlobal->McbAllocated > (FFSGlobal->MaxDepth << 1))
		Extra = FFSGlobal->McbAllocated - 
			(FFSGlobal->MaxDepth << 1) +
			FFSGlobal->MaxDepth;
	...
	PFFS_MCB Mcb)
{
	...
	if (FFSLoadInode(Vcb, Mcb->Inode, &FFSIno))
	{
		PFFS_INODE pTmpInode = ExAllocatePool(PagedPool, sizeof(FFS_INODE));
		if (!pTmpInode)
		{
			goto errorout;
		}

		RtlCopyMemory(pTmpInode, &FFSIno, sizeof(FFS_INODE));
		Fcb = FFSAllocateFcb(Vcb, Mcb, pTmpInode);
		if (!Fcb)
		{
